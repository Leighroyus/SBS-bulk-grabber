# *** SBS Bulk Grabber v0.1 - Leigh Sullivan 2017-07-02  ***
...
# *** function to convert UTC as seconds into datetime format (NOT USED) ***
def _utc_to_datetime(utc_timestamp):
    return datetime.datetime.utcfromtimestamp(utc_timestamp/1000).strftime('%Y-%m-%d %H:%M:%S')

# *** function to get how many web requests are needed ***
def _num_of_sub_lists(size):
    return int(math.ceil(float(size) / 3000))
...
def create_url_list(list_size):
    if list_size > 9000:
        list_size = 9000
    url_list = []
    if _num_of_sub_lists(list_size) == 1:
        url_list.append(API_URL_BASE + "1-" + str(list_size))
    if _num_of_sub_lists(list_size) == 2:
        url_list.append(API_URL_BASE + "1-3000")
        url_list.append(API_URL_BASE + "3001-" + str(list_size))
    if _num_of_sub_lists(list_size) == 3:
        url_list.append(API_URL_BASE + "1-3000")
        url_list.append(API_URL_BASE + "3001-6000")
        url_list.append(API_URL_BASE + "6001-" + str(list_size))
    return (url_list)
...
def create_processed_list(raw_list):
    ...
    for single_entry in raw_list:
        ...
        SBS_single_tv_object.num_index_ID = single_entry["pl1$pilatId"]
...
url_list = create_url_list(10000)
...
SBS_sorted_list = sorted(SBS_show_list, key=lambda SBS_show_list: SBS_show_list.num_index_ID, reverse=False)
...
for index, show in enumerate(SBS_sorted_list):
    ...
    print "Loop ID: " + str(index) + " Group ID: " + str(
        show.num_group_ID) + " Index ID: " + show.num_index_ID + ", Title: " + show.str_title # + " - Levenshtein match: %" + str(lev)
